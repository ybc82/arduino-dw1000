function pf = pf_init_3D()

%% Initialization
% Constants
MAX_SPEED = 1;      % 1m/s
TIME_STEP = 0.2;    % 0.2s per cycle
% anchor1 = [-1.7; 3.58; 0];    % position of anchor K, [X; Y; Z] (m)
% anchor2 = [-0.37; -3.85; 0]; 
% anchor3 = [1.143; 1.127; 0];
anchor1 = [2; 0; 0];
anchor2 = [1; 0; 0];
anchor3 = [3; 0; 0];
...
p_sys_noise     = @(u) normpdf(u(1), 0, sigma_u(1))*normpdf(u(2), 0, sigma_u(2))*normpdf(u(3), 0, sigma_u(3));
gen_sys_noise   = @(u) [normrnd(0, sigma_u(1)); normrnd(0, sigma_u(2)); normrnd(0, sigma_u(3))];     % sample from p_sys_noise (return column vector)

% PDF of observation noise and noise generator function
nv = 3;             % size of the vector of observation
pf.sigma_v = ones(nv, 1) * 1 / 3; % 10 cm for 3 sigma % CHANGE THIS BACK TO 0.1/3
